...
public:
  ...
  BirdChaser(int mp3TxPin, int mp3RxPin, int buttonPin)
    : mp3(mp3TxPin, mp3RxPin), buttonPin(buttonPin) {
  ...
  void handleVolumeButtons() {
    ...
    if (volumeUpReading == HIGH && (currentMillis - lastVolumeUpDebounceTime > debounceDelay)) {
      lastVolumeUpDebounceTime = currentMillis;  /gola/ Update the debounce time
